...
#           Written by:
#           Madison Menezes (MacID: menezm4)
#           and
#           Saransh Bedi (MacID: bedis9)

#           Opening a text file and inserting the column headers
#           for raw data and processed data in the text file and shell.
#           Also creating an instance of the gas sensor and push button 
        
            write_file = open('text_file.txt','w')
            appended_data = open('text_file.txt','a')
            appended_data.write("CO (raw)" + "\t" + "CO (average)" + "\n")
            appended_data.close()
            sensor = Gas_Sensor()
            push_button = Button(21)
            print("RAW DATA (CO)", "\t", "ROLLING AVERAGE (CO)", "\t", "BUZZER STATUS")

#           Infinite loop that prints raw CO levels and rolling average
#           of last 5 CO levels. The shell will only display raw data
#           until there are enough data points to calculate a rolling
#           average. Also, all additional functions are called here.
#           Loop will break if button is held  

            while True:
                if push_button.is_pressed:
                    break
                else:
                    Carbon_monoxide = sensor.CO_gas()
                    raw_data.append(Carbon_monoxide)
                    if len(raw_data)<6:
                        print(Carbon_monoxide, "\t", "\t", "none")
                        time.sleep(0.5)
                    else:
                        average = get_average()
                        print(Carbon_monoxide, "\t", "\t", average[-1])
                        text_file()
                        trend()
                        buzzer()
                        time.sleep(0.5)
...
#   Initializing two empty lists for raw data and rolling average.
#   Also creating an instance of the gas sensor outside of the main function
...
#   FUNCTION 1:
#   Calculates the rolling average of the last 5 values of CO and appends it to
#   a list of processed data. Returns the list

def get_average():
    total_sum = 0
    for i in range(-6,-1):
        total_sum += raw_data[i]
    average = total_sum / 5
    processed_data_list.append(average)
    return processed_data_list 

#   FUNCTION 2:
#   Appends raw data and rolling average to a text file
#   by converting integer indecies into strings in two seperate columns 

def text_file():
    Carbon_monoxide = sensor.CO_gas()
    average = get_average()
    string_average = str(average[-1])
    appended_data = open('text_file.txt','a')
    appended_data.write((str(Carbon_monoxide)) + "\t" + "\t" + string_average + "\n")

#   Function 3: Process 1 
#   Activates a vibrator for varying amounts of time depending on different
#   thresholds of the rolling average of CO. Duration of vibration decreases
#   as CO increases so that a rapid vibrating sensation is felt under
#   severe circumstances. Function also displays the status of the output
#   devices in the shell which was done in correspondence to the requirements
#   listed in the project module
...
#   Function 4: Process 2 
#   Calculates the direction of the slope of CO values and displays
#   whether or not the CO levels are rising or falling on an LCD screen.
#   Once there are enough indecies in the rolling average to calculate 
#   two seperate slopes, the LCD will only display a print statement if
#   the direction of the trend has changed from what it previously was. This was
#   done to prevent the screen from displaying repetitive print statements.
#   If the CO levels are changing by a range less than 1, the LCD displays
#   that the CO levels are no longer changing.

def trend():
    average = get_average()
    if len(average) >=5 and len(average)<8:
        trend = (average[-1] - average[-5])
        if trend<-1:
            print("\n", "Carbon monoxide levels are decreasing", "\n")
        elif trend>1:
            print("\n", "Carbon monoxide levels are increasing","\n" )
        else:
            print("\n", "Carbon monoxide levels are staying", "\n", "   the same at about", average[-1], "ppm","\n" )
    elif len(average) >=8:
        previous_trend = (average[-6] - average[-8])
        trend = (average[-1] - average[-5])
        if trend<-1 and previous_trend>-1:
            print("\n", "Carbon monoxide levels are decreasing", "\n")
        elif trend>1 and previous_trend<1:
            print("\n","Carbon monoxide levels are increasing","\n" )
        elif (trend>-1 and trend<1) and (previous_trend>1 or previous_trend<-1):
            print("\n", "Carbon monoxide levels are staying", "\n", "   the same at about",  average[-1], "ppm","\n" )
